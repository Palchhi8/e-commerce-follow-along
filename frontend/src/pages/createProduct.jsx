...
const CreateProduct = () => {
  ...
  useEffect(() => {
    if (isEdit) {
      axios
        .get(`http://localhost:5050/api/v2/product/product/${id}`)
        .then((response) => {
          ...
          if (p.images && p.images.length > 0) {
            setPreviewImages(p.images.map((imgPath) => `http://localhost:5050${imgPath}`));
          }
        })
        .catch((err) => {
    ...
    try {
      ...
      if (isEdit) {
        response = await axios.put(
          `http://localhost:5050/api/v2/product/update-product/${id}`,
          formData,
    ...
    <div className="w-[90%] max-w-[500px] bg-white shadow h-auto rounded-[4px] p-4 mx-auto">
      ...
      <form onSubmit={handleSubmit}>
        <div className="mt-4">
          ...
          <input type="email" value={email} className="w-full p-2 border rounded"
        ...
        <div>
          ...
          <input type="text" value={name} className="w-full p-2 border rounded"
            onChange={(e) => setName(e.target.value)} placeholder="Enter product name" required />
        </div>
        <div className="mt-4">
          <label className="pb-1 block">Description <span className="text-red-500">*</span></label>
          <textarea value={description} className="w-full p-2 border rounded"
            onChange={(e) => setDescription(e.target.value)} placeholder="Enter product description" rows="4" required />
        </div>
        <div className="mt-4">
        ...
        <div className="mt-4">
          ...
          <input type="text" value={tags} className="w-full p-2 border rounded"
            onChange={(e) => setTags(e.target.value)} placeholder="Enter product tags" />
        </div>
        <div className="mt-4">
          <label className="pb-1 block">Upload {isEdit ? "New" : ""} Images {isEdit ? "(optional)" : ""}:</label>
